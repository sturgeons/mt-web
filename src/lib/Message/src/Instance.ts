...
const createInstance = (cfg: any) => {
    ...
    const height = 0.8 // 单个消息框高度
    const messageList: any = document.getElementsByClassName('message')
    messageNode.style.top = `${messageList.length * height}rem`
    // 2、创建实例并挂载到 body
...
// 挂载实例并追加到 body 结尾
    ...
    const resetMsgTop = () => {
        for (let i = 0; i < messageList.length; i++) {
            messageList[i].style.top = `${i * height}rem`
        }
    }
    return app
